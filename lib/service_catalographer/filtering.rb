# ServiceCatalographer: lib/service_catalographer/filtering.rb
#
...
module ServiceCatalographer
  ...
  # This is done twice so that an initial one is created
      ...
      unless [ :t, :tag, :tag_s, :tag_ops, :tag_rms, :tag_ins, :tag_outs, :c ].include?(filter_type)
        name = case filter_type
          ...
          when :p, :asp, :sosp
            ...
            (s.nil? ? "(unknown provider)" : ServiceCatalographer::Util.display_name(s, false))
          when :su, :sou
            u = User.find_by_id(filter_id)
            (u.nil? ? "(unknown user)" : ServiceCatalographer::Util.display_name(u, false))
          when :sr, :sor
            r = Registry.find_by_id(filter_id)
            (r.nil? ? "(unknown registry)" : ServiceCatalographer::Util.display_name(r, false))
          when :soa
            a = Agent.find_by_id(filter_id)
            (a.nil? ? "(unknown agent)" : ServiceCatalographer::Util.display_name(a, false))
          when :attrib
            a = AnnotationAttribute.find_by_id(filter_id)
            (a.nil? ? "(unknown annotation attribute)" : a.identifier)
          when :as
            a = Service.find_by_id(filter_id)
            (a.nil? ? "(unknown service)" : ServiceCatalographer::Util.display_name(a, false))
          when :asd
          ...
          when :ars
            ...
            (a.nil? ? "(unknown REST service)" : ServiceCatalographer::Util.display_name(a, false))
          when :ass
            a = SoapService.find_by_id(filter_id)
            (a.nil? ? "(unknown SOAP service)" : ServiceCatalographer::Util.display_name(a, false))
          when :asop
            a = SoapOperation.find_by_id(filter_id)
            (a.nil? ? "(unknown SOAP operation)" : ServiceCatalographer::Util.display_name(a, false))
          when :asin
            a = SoapInput.find_by_id(filter_id)
            (a.nil? ? "(unknown SOAP input)" : ServiceCatalographer::Util.display_name(a, false))
          when :asout
            a = SoapOperation.find_by_id(filter_id)
            (a.nil? ? "(unknown SOAP output)" : ServiceCatalographer::Util.display_name(a, false))
          when :arm
            a = RestMethod.find_by_id(filter_id)
            (a.nil? ? "(unknown REST method)" : ServiceCatalographer::Util.display_name(a, false))
          when :arp
            a = RestParameter.find_by_id(filter_id)
            (a.nil? ? "(unknown REST parameter)" : ServiceCatalographer::Util.display_name(a, false))
          when :arr
            a = RestRepresentation.find_by_id(filter_id)
            (a.nil? ? "(unknown REST representation)" : ServiceCatalographer::Util.display_name(a, false))
          when :arres
            a = RestResource.find_by_id(filter_id)
            (a.nil? ? "(unknown REST resource)" : ServiceCatalographer::Util.display_name(a, false))
        end
      end

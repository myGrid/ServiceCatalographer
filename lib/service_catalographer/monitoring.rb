# ServiceCatalographer: lib/service_catalographer/monitoring.rb
#
  ...
  # ==========
    ...
    class CheckUrlStatus
      ...
      def self.check_url_status(url)
        ...
        check =  ServiceCatalographer::AvailabilityCheck::URLCheck.new(url)
        if check.available?
      ...
      def self.generate_soap_fault(endpoint)
        ...
        ep =  ServiceCatalographer::AvailabilityCheck::SoapEndPoint.new(endpoint)
        if ep.available?
      ...
      #     ServiceCatalographer::Monitoring::CheckUrlStatus.run :service_ids => [1,2,3]
      def self.run (*params)
        options = params.extract_options!.symbolize_keys
        ...
        if options[:service_ids] and options[:all]
         ...
         if not options[:service_ids] and not options[:all]
          puts "Please run"
          puts "ServiceCatalographer::Monitoring::CheckUrlStatus.run :all => true"
          puts "to run monitoring on all the services OR"
          puts "ServiceCatalographer::Monitoring::CheckUrlStatus.run :service_ids => [some, service, ids]"

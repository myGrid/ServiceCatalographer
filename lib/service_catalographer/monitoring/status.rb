# ServiceCatalographer: lib/service_catalographer/monitoring/status.rb
#
...
module ServiceCatalographer
  module Monitoring
    ...
    class ServiceStatus < Status
      ...
      # 1 Ignore all ServiceTests NOT run by the ServiceCatalogue
      # 2 For all ServiceTests that are run by the ServiceCatalogue, go through the latest TestResults
      ...
      def populate
        ...
        test_results = activated_service_tests.map{|st| st.latest_test_result if ServiceCatalographer::Monitoring::INTERNAL_TEST_TYPES.include?(st.test_type)}.compact

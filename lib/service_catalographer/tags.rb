# ServiceCatalographer: lib/service_catalographer/tags.rb
#
  ...
  module Tags
    ...
    def self.get_service_ids_for_tag(tag_name)
      ...
      @@models_for_search.each do |type|
        result_hash[type] = ServiceCatalographer::Mapper.process_compound_ids_to_associated_model_object_ids(results.map{|r| ServiceCatalographer::Mapper.compound_id_for(r['type'], r['id']) }, type).uniq
    ...
    def self.annotations_to_tags_structure(annotations)
      ...
      annotations.each do |ann|
        if ann.attribute_name.downcase == "tag" && ann.value_type == "Tag"
          ...
          submitter_compound_id = ServiceCatalographer::Mapper.compound_id_for(ann.source_type, ann.source_id)

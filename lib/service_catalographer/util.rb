# ServiceCatalographer: lib/service_catalographer/util.rb
#
  ...
  module Util
    ...
    def self.url_location_lookup(url)
      begin
        ...
        Rails.logger.error("Method ServiceCatalographer::Util.url_location_lookup - timeout occurred when attempting to perform DNS resolution.")
        Rails.logger.error($!)
        return nil
      rescue Exception => ex
        Rails.logger.error("Method ServiceCatalographer::Util.url_location_lookup errored. Exception:")
        Rails.logger.error($!)
        return nil
      end
    end
    ...
    def self.ip_geocode(ip)
      ...
      begin
        ...
        Rails.logger.error("Method ServiceCatalographer::Util.ip_geocode - timeout occurred when attempting to get info from HostIp.")
        Rails.logger.error($!)
        return geoloc
      rescue Exception => ex
        Rails.logger.error("Method ServiceCatalographer::Util.ip_geocode - failed on call to HostIp. Exception:")
        Rails.logger.error($!)
        return geoloc
      end
      ...
      items.each do |item|
        if item.is_a?(ActiveRecord::Base)
          if item.is_a?(model)
            ...
            case model.to_s
              when "Service"
                Rails.logger.info "ServiceCatalographer::Util.discover_model_objects_from_collection - model required = Service;"
                case item
                  when ServiceVersion, 
                       ServiceDeployment, 
                       SoapService,
                  ...
                  when Annotation
                    ...
                    Rails.logger.info "ServiceCatalographer::Util.discover_model_objects_from_collection - model required = Service; item type = #{item.class.name}, no way to get a top level Service for the item."
                end
            end
          end
        end
    ...
    def self.find_wsdl_file_for(obj)
      return nil unless ServiceCatalographer::Mapper::SOAP_SERVICE_STRUCTURE_MODELS.include?(obj.class)

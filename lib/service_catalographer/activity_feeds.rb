# ServiceCatalographer: lib/service_catalographer/activity_feeds.rb
#
  ...
  module ActivityFeeds
    ...
    def self.activity_logs_for(location, *args)
      ...
      begin
        ...
        cache_key = ServiceCatalographer::CacheHelper.cache_key_for(:activity_log_entries, location, options[:scoped_object_type], options[:scoped_object_id], options[:style])
        ...
        ServiceCatalographer::Util.log_exception(ex, :error, "Failed to run ActivityFeeds.activity_logs_for location: #{location}")
      end

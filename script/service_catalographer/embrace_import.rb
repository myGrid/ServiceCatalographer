...
# This script imports the data from the EMBRACE registry into ServiceCatalogue.
...
class EmbraceImporter
  ...
  def initialize(args)
    ...
    # Get or create the ServiceCatalographer agent, which we will be using as the annotation source...
    @biocat_agent = Agent.find_by_name("servicecatalographer")
    if @biocat_agent.nil?
      @biocat_agent = Agent.create(:name => "servicecatalographer",
                                   :display_name => "ServiceCatalographer")
    end
      ...
      User.transaction do
        ...
        @data.users.each do |user_id, user|
          ...
          if existing
            ...
            Relationship.create(:subject => existing, :predicate => "ServiceCatalogue:alsoIn", :object => @registry_source)
            ...
            if u.save
              ...
              unless @options[:test]
                UserMailer.reset_password(u, "www.servicecatalographer.org").deliver
              end
              
              puts "INFO: new user added to DB and a reset password email has been sent"
              stats["ids_of_users_new"] << u.id
              
              Relationship.create(:subject => u, :predicate => "ServiceCatalogue:origin", :object => @registry_source)
        ...
        @data.services.each do |service_id, service|
          ...
          if submitter.nil?
            ...
            if biocat_service
              ...
              Relationship.create(:subject => biocat_service, :predicate => "ServiceCatalogue:embraceOriginalSubmitter", :object => submitter)
            end
          end
    ...
    begin
      ...
      if (existing_service = SoapService.check_duplicate(wsdl_url, "")).nil?
        ...
        if new_service_success
          ...
          if new_service_success
            Relationship.create(:subject => soap_service.service(true), :predicate => "ServiceCatalogue:origin", :object => @registry_source)
        ...
        puts "INFO: existing matching service found (ID: #{existing_service.id}, WSDL URL: '#{wsdl_url}', Submitter: #{ServiceCatalographer::Util.display_name(existing_service.submitter)})."
        ...
        if existing_service.submitter == @registry_source
          ...
          Relationship.create(:subject => existing_service, :predicate => "ServiceCatalogue:origin", :object => @registry_source)
        else
          Relationship.create(:subject => existing_service, :predicate => "ServiceCatalogue:alsoIn", :object => @registry_source)
        end
  ...
  def process_rest_service(service, submitter, stats)
    ...
    begin
      ...
      if (existing_service = RestService.check_duplicate(endpoint)).nil?
        ...
        if new_service_success
          Relationship.create(:subject => rest_service.service(true), :predicate => "ServiceCatalogue:origin", :object => @registry_source)
        ...
        puts "INFO: existing matching service found (ID: #{existing_service.id}, Endpoint URL: '#{endpoint}', Submitter: #{ServiceCatalographer::Util.display_name(existing_service.submitter)})."
  ...
  def sync_annotation(annotatable, attribute, value, original_submitter, stats)
    ...
    unless annotation.nil?
      Relationship.create(:subject => annotation, :predicate => "ServiceCatalogue:origin", :object => @registry_source)
    end
  end

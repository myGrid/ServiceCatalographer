...
# database into the ServiceCatalogue's database. The scripts goes through
...
# test script data, if a corresponding service is found in ServiceCatalogue,
...
#
#TODO : tests script should be attributed to the user from EMBRACE, not the submitter in ServiceCatalogue

# Usage :
#   runs script on ServiceCatalogue development database using these arguments for embrace DB
#   ruby script/service_catalographer/embrace_test_scripts_import.rb --environment=<env> --host=<host> --port=<port> --user=<user> --database=<db> --password=<password>
          ...
          unless wsdl_url.strip.blank?
            begin
              ...
              if (existing_service = SoapService.check_duplicate(wsdl_url, "")).nil?
                # service not available in ServiceCatalogue
                # nothing to do
                puts "INFO: Embrace Service with ID #{id} not found in ServiceCatalogue"
                ...
                if tests.num_rows > 0
                  puts "INFO: creating test in ServiceCatalogue"
                  tests.each do |script|
                    if create_biocat_test_script(script, existing_service, conn)
                      puts "INFO: Created a new test script in ServiceCatalogue"
                    end
                  end
                end
              end
            ...
            begin
              ...
              unless endpoint.nil?
                if (existing_service = RestService.check_duplicate(endpoint)).nil?
                  puts "INFO: Service does not exist in ServiceCatalogue"
                  @stats.counts[:services_not_found_scripts] += test_scritpts_for_service(id,conn).num_rows
                else
           ...
           begin
              if service_test.save!
                ...
                relationship = Relationship.new(:subject_type => a_test.service_test.class.name,
                                                    ...
                                                    :predicate    => 'ServiceCatalogue:sameAs',
...
if config.values.include?(nil)
  ...
  puts "ruby script/service_catalographer/embrace_test_scripts_import.rb --environment=<env> --host=<host> --port=<port> --user=<user> --database=<db> --password=<password>"
  puts "Now Exiting ..."
  exit(0)
end

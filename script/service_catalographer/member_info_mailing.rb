...
# Send an email to members of the ServiceCatalogue that have consented to
  ...
  def mail_members
    ...
    members.each do |member|
      if member.respond_to?(:email) && member.email && member.receive_notifications && member.activated_at
        Delayed::Job.enqueue(ServiceCatalographer::Jobs::MemberInfoEmail.new(@subject, @message, member.email), :priority => 0, :run_at => 5.seconds.from_now)
      end
    end
  end
